...
Where :user_id/:user should exist in database. Similarly if devise gem is used for authentication Add following line in the code:
...
of times to hit a action is already supplied in parametes (*3 or *4 in parameters) then it overwrites the 'repete' for that action
